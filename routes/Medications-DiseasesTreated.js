...
//id = medID
  mysql.pool.query("SELECT medID, medName FROM Medications", function(err, resu, /*fields*/){
      if(err){
          console.log("err");
   ...
   console.log("Inserting Medication Data Into Medications Table...");
    ...
    res.redirect('/Medications');
        ...
        function getDiseasesTreated(res, mysql, context, complete){
          ...
          mysql.pool.query("SELECT id, medID, disease FROM DiseasesTreated", function(err, returned, fields){
          if(err){
              console.log("err");
              res.write(JSON.stringify(err));
              res.end();
